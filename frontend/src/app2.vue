...
// ==================== 响应式状态管理 ====================
const currentTab = ref<'log' | 'stats'>('log');
const version = ref<string>(packageInfo.version);
const activeModal = ref<RecordType | null>(null);
const showBackendLog = ref<boolean>(false);

// 过滤与时间区间
const timelineFilterDate = ref<string>('2026-05-20');
const milkChartStartDate = ref<string>('');
const milkChartEndDate = ref<string>('');
const selectedChartDay = ref<DailyMilkData | null>(null);
const tempChartStartDate = ref<string>('');
const tempChartEndDate = ref<string>('');
const selectedTempPoint = ref<TempPoint | null>(null);
...
const form = ref({
  ...
  bottle_volume_ml: 90,
  sugar_value: 4.5,
  sugar_period: 'before_feed' as SugarPeriod,
  diaper_status: 'wet' as DiaperStatus,
  weight_kg: 5.20,
  height_cm: 56.5,
  medication_type: 'ad' as 'ad' | 'probiotics' | 'mixed' | 'other',
  temperature: 36.8,
  remark: ''
});
...
onMounted(() => {
  ...
  // 2. 设定定时器：每 10 秒自动向云端拉取一次最新记录，保持双人数据绝对同步
  const timer = setInterval(() => {
    // 只有在没有打开录入弹窗时才自动刷新，避免打扰用户写备注
...
const babyAgeChinese = computed<string>(() => {
...
// ==================== 物理后端请求层 (Fetch API) ====================
const pushLog = (method: ApiLog['method'], url: string, status: number, data: any) => {
  const now = new Date();
...
// ==================== 计算属性 (TS 强类型约束) ====================
const filteredRecords = computed<BabyRecord[]>(() => {
...
const filteredDailyMilkData = computed<DailyMilkData[]>(() => {
...
const todayFeedVolume = computed<number>(() => {
  const todayStr = '2026-05-20';
  return records.value
      .filter(r => r.timestamp.slice(0, 10) === todayStr && (r.type === 'breast_bottle' || r.type === 'bottle_formula'))
      .reduce((acc, curr) => acc + (curr.detail.bottle_volume_ml || 0), 0);
});

const todayFormulaVol = computed<number>(() => {
  const todayStr = '2026-05-20';
  return records.value.filter(r => r.timestamp.slice(0, 10) === todayStr && r.type === 'bottle_formula').reduce((acc, curr) => acc + (curr.detail.bottle_volume_ml || 0), 0);
});

const todayBreastBottleVol = computed<number>(() => {
  const todayStr = '2026-05-20';
  return records.value.filter(r => r.timestamp.slice(0, 10) === todayStr && r.type === 'breast_bottle').reduce((acc, curr) => acc + (curr.detail.bottle_volume_ml || 0), 0);
});

const todayBreastLiveCount = computed<number>(() => {
  const todayStr = '2026-05-20';
  return records.value.filter(r => r.timestamp.slice(0, 10) === todayStr && r.type === 'breast_live').length;
});

const todayDiaperDirty = computed<number>(() => {
  const todayStr = '2026-05-20';
  return records.value.filter(r => r.timestamp.slice(0, 10) === todayStr && r.type === 'diaper' && (r.detail.diaper_status === 'dirty' || r.detail.diaper_status === 'mixed')).length;
});

const todayDiaperWet = computed<number>(() => {
  const todayStr = '2026-05-20';
  return records.value.filter(r => r.timestamp.slice(0, 10) === todayStr && r.type === 'diaper' && (r.detail.diaper_status === 'wet' || r.detail.diaper_status === 'mixed')).length;
});

const timeSinceLastFeed = computed<string>(() => {
  const feeds = records.value.filter(r => ['breast_live', 'breast_bottle', 'bottle_formula'].includes(r.type));
  if (feeds.length === 0) return '暂无记录';
  const nowSimulated = new Date('2026-05-20T10:37:00');
  const last = new Date(feeds[0].timestamp);
  const diffMs = nowSimulated.getTime() - last.getTime();
...
const latestBloodSugar = computed<{ value: number | null; period?: SugarPeriod }>(() => {
...
const growthChartPoints = computed<GrowthPoint[]>(() => {
...
const weightChartPath = computed<string>(() => growthChartPoints.value.reduce((path, p, idx) => path + `${idx === 0 ? 'M' : 'L'} ${p.x} ${100 - p.yWeight}`, ''));
const heightChartPath = computed<string>(() => growthChartPoints.value.reduce((path, p, idx) => path + `${idx === 0 ? 'M' : 'L'} ${p.x} ${100 - p.yHeight}`, ''));

const sugarChartPoints = computed<SugarPoint[]>(() => {
...
const sugarChartPath = computed<string>(() => sugarChartPoints.value.reduce((path, p, idx) => path + `${idx === 0 ? 'M' : 'L'} ${p.x} ${100 - p.y}`, ''));
const sugarChartAreaPath = computed<string>(() => {
...
const tempChartPoints = computed<TempPoint[]>(() => {
...
const tempLimitLineY = computed<number>(() => {
...
const donutChartStyle = computed<string>(() => {
...
const getRecordBadgeStyle = (record: BabyRecord) => {
  if (record.type === 'blood_sugar') {
...
<template>
  <!-- 全屏自适应容器 (整体底色调谐，通过 h-screen overflow-hidden 锁死滚动导致的抖动) -->
  <div class="w-full h-screen bg-[#F5F3ED] flex flex-col relative overflow-hidden antialiased text-[#4A3D3C] max-w-md mx-auto pb-16">

    <!-- 顶部渐变状态栏 (Header 宽度与底部对齐) -->
    <header class="bg-gradient-to-b from-[#FFF4D4] via-[#FFF9E4] to-[#F5F3ED] px-6 pt-6 pb-4 flex-shrink-0">
      ...
      <!-- 核心数据看板卡片 (具有高保真投影和立体感) -->
      <div class="bg-white rounded-2xl p-4 shadow-[0_8px_30px_rgba(160,150,130,0.12)] border border-amber-100/10 flex justify-between items-center">
        <div class="space-y-1">
    ...
    <!-- 主展示滑动区域 (两页卡片宽度彻底拉直、完美对齐，滚动条完全隐藏) -->
    <main class="flex-grow overflow-y-auto px-6 no-scrollbar bg-[#F5F3ED] w-full">

      <!-- ==================== Tab 1: 去记录 ==================== -->
      <div v-if="currentTab === 'log'" class="space-y-5 pt-2 w-full">

        <!-- 快捷记录大白色卡片：参考看数据排版，彻底消除左右碎空间 -->
        ...
        <div class="bg-white rounded-3xl p-5 shadow-[0_10px_30px_rgba(160,150,130,0.1)] border border-gray-100/50 space-y-4 w-full">
          <div class="flex justify-between items-center border-b border-gray-50 pb-3">
            ...
            <button @click="loadMockData" class="text-xs text-[#FF9E9E] hover:underline font-bold self-end pb-1">导入演示数据</button>
          </div>
      ...
      <div v-if="currentTab === 'stats'" class="space-y-5 pt-2 w-full">
        ...
        <div class="bg-white rounded-3xl p-5 shadow-[0_10px_30px_rgba(160,150,130,0.1)] border border-gray-100/50 space-y-4 w-full">
          ...
          <div v-else class="text-center py-10 text-xs text-gray-400 border border-dashed border-gray-100 rounded-2xl">
            📭 暂无统计区间数据
          </div>
        </div>
        ...
        <div class="bg-white rounded-3xl p-5 shadow-[0_10px_30px_rgba(160,150,130,0.1)] border border-gray-100/50 w-full">
          ...
          <div class="flex justify-between text-[10px] text-gray-400 mt-2 px-2 font-bold" v-if="sugarChartPoints.length > 1">
            <span>最早记录</span>
            <span>最新记录</span>
          </div>
        </div>
    ...
    <!-- 底部 Tab 页标签 (固定于底部，完美离线 SVG) -->
    <nav class="absolute bottom-0 left-0 right-0 h-16 bg-white border-t border-gray-100 flex justify-around items-center z-30 shadow-[0_-4px_12px_rgba(0,0,0,0.03)] max-w-md mx-auto">
          ...
          <div v-if="activeModal === 'blood_sugar'" class="space-y-4">
            ...
            <div class="space-y-2">
              ...
              <div class="grid grid-cols-2 gap-2">
                <button type="button" @click="form.sugar_period = 'before_feed'" class="py-2.5 rounded-xl text-xs font-bold border transition-colors" :class="form.sugar_period === 'before_feed' ? 'bg-rose-500 text-white border-rose-500' : 'bg-gray-50 text-gray-500 border-gray-100'">
                  空腹 / 喂奶前
                </button>
                <button type="button" @click="form.sugar_period = 'after_feed_1h'" class="py-2.5 rounded-xl text-xs font-bold border transition-colors" :class="form.sugar_period === 'after_feed_1h' ? 'bg-rose-500 text-white border-rose-500' : 'bg-gray-50 text-gray-500 border-gray-100'">
                  喂奶后 1 小时
                </button>
                <button type="button" @click="form.sugar_period = 'after_feed_2h'" class="py-2.5 rounded-xl text-xs font-bold border transition-colors" :class="form.sugar_period === 'after_feed_2h' ? 'bg-rose-500 text-white border-rose-500' : 'bg-gray-50 text-gray-500 border-gray-100'">
                  喂奶后 2 小时
                </button>
                <button type="button" @click="form.sugar_period = 'random'" class="py-2.5 rounded-xl text-xs font-bold border transition-colors" :class="form.sugar_period === 'random' ? 'bg-rose-500 text-white border-rose-500' : 'bg-gray-50 text-gray-500 border-gray-100'">
                  随机测量
                </button>
              </div>
            </div>
          ...
          <div v-if="activeModal === 'diaper'" class="space-y-3">
            ...
            <div class="grid grid-cols-3 gap-3">
              <button type="button" @click="form.diaper_status = 'wet'" class="py-4 rounded-2xl border text-center transition-all flex flex-col items-center gap-1.5" :class="form.diaper_status === 'wet' ? 'bg-sky-500 text-white border-sky-500 scale-95 shadow-sm' : 'bg-gray-50 text-gray-600 border-gray-100'">
                ...
                <span class="text-xs font-bold">只有尿尿</span>
              </button>
              <button type="button" @click="form.diaper_status = 'dirty'" class="py-4 rounded-2xl border text-center transition-all flex flex-col items-center gap-1.5" :class="form.diaper_status === 'dirty' ? 'bg-amber-700 text-white border-amber-700 scale-95 shadow-sm' : 'bg-gray-50 text-gray-600 border-gray-100'">
                <span class="text-xl">💩</span>
                <span class="text-xs font-bold">只有拉屎</span>
              </button>
              <button type="button" @click="form.diaper_status = 'mixed'" class="py-4 rounded-2xl border text-center transition-all flex flex-col items-center gap-1.5" :class="form.diaper_status === 'mixed' ? 'bg-emerald-600 text-white border-emerald-600 scale-95 shadow-sm' : 'bg-gray-50 text-gray-600 border-gray-100'">
                <span class="text-xl">🌟</span>
                <span class="text-xs font-bold">混合粑粑</span>
              </button>
            </div>
          </div>
